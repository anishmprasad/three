  ...
  function checkBufferGeometryIntersection(
    ...
    if (intersection) {
      ...
      intersection.face = new THREE.Face3(
        ...
        b,
        c,
        THREE.Triangle.normal(vA, vB, vC)
      );
...
export function start(elementId, data, editorMode) {
  ...
  for (let i = 0; i < data.slides.length; i++) {
    ...
    if (!data.slides[i].envLight) {
      data.slides[i].envLight =
        data.assets.envLight ||
        "/assets-qbix/F16EFB92-4B2C-46EB-8F86-8AC2322E4FAE.env";
    }
            ...
            if (slideData.layers[uuid] === true) {
              ...
              Object.keys(newLayers2D[newUuid].objects).forEach(objUuid => {
                ...
                let oldMeshLineParentUuid =
                  newLayers2D[newUuid].objects[uuidMap[objUuid]]
                    .meshLineParentUuid;
                if (
                  oldMeshLineParentUuid &&
                  !data.layers.layers2D[oldMeshLineParentUuid]
                ) {
                  delete newLayers2D[newUuid].objects[uuidMap[objUuid]]
                    .meshLineParentUuid;
                }
                if (
                  oldMeshLineParentUuid &&
                  ...
                  Object.keys(
                    newLayers2D[newMeshLineParentUuid].objects
                  ).forEach(lineUuid => {
  ...
  Q3.splash.pop(elementId, () => {
    ...
    if (cameraLimits.horizontalAngleLimitEnabled) {
      Q3.controls.minAzimuthAngle =
        (cameraLimits.minAzimuthAngle * Math.PI) / 180;
      Q3.controls.maxAzimuthAngle =
        (cameraLimits.maxAzimuthAngle * Math.PI) / 180;
    }
  ...
  Q3.loadEnvLight = function(url) {
    ...
    function readBuffer(buffer, ref) {
      ...
      var base64String = base64ArrayBuffer(
        buffer.slice(ref.pos, ref.pos + size)
      );
      ref.pos += size;
