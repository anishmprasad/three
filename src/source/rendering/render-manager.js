...
class RenderManager {
	...
	setSize(w, h, updateStyle) {
		const pixelRatio =
			Q3.browserCapabilities.renderQuality >= RenderQuality.NORMAL ? window.devicePixelRatio || 1 : 1;
	...
	compose(delta) {
		...
		function selectVisibleObjects() {
			...
			if (Q3.data.slides[Q3.slide].layers.Renderer)
				[Network.world, { scene: Q3.gateNetwork.parent }].concat(Network.gates).forEach(world => {
					world.scene &&
